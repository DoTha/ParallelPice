...
import Control_Problems.Theano_LQ1 as DCP
...
#instantiate objects
controller = NN.Controller(10,100,2,2,0.001,"adam") #build a neural network which serves as controller. parameters: [depth,width,c_inp_dim,control_dim,learning_rate]
...
for i in range(rollout_epochs):
    ...
    if comm.Get_rank() == 0:
        ...
        print "EffSS = ", ParDataGen.EffSS()
    val_err = parallel_trainer.train_network(2, 300, lambda x: x<0.01)
    if comm.Get_rank() == 0: val_err_store[i] = val_err


my_save_local("val_err_store",file_identifier)
my_save_local("EffSS_store",file_identifier)
